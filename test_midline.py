...
def finish_correction():
    ...
    round(head_parameter.iloc[9:17, -1] += midline, 0)
    round(head_parameter.iloc[17:25, -1] += midline, 0)
    worksheet.clear()  # Optional: Use with caution, can clear the entire sheet
    set_with_dataframe(worksheet, head_parameter)  # Update the sheet

    mice = np.array(file.iloc[0,:])
    num_mice = len(mice[60:]) # count the number of mice

    mouse_index = np.where(mice == mouse_id)[0][0]  # Find the index of the mouse
    mousefile = file.iloc[0:53,mouse_index]
    mousefile.index = file.iloc[0:53,1].to_list()
    ...
    leftRidge = file.iloc[9:17,60:]
    leftRidge.columns = file.iloc[0,60:]
    rightRidge = file.iloc[17:25,60:]
    rightRidge.columns = file.iloc[0,60:]
    leftRidgeZ = file.iloc[25:33,60:]
    leftRidgeZ.columns = file.iloc[0,60:]
    rightRidgeZ = file.iloc[33:41,60:]
    rightRidgeZ.columns = file.iloc[0,60:]

    animalWeight = file.iloc[0,60:]
    LeftEarBarInitial = file.iloc[3,60:]
    RightEarBarInitial = file.iloc[4,60:]
    NoseDVposition = file.iloc[8,60:]
    RCSlambdaDistance = file.iloc[7,60:]
